...
void print_list(doubll2d<int>* A)
{
    printf("\n\n");
    printf("----------------------------------------------------------------------\n");
    printf("rows: %zu, ", A->dim_row);
    printf("cols: %zu\n", A->dim_col);
    if (A->dim_row)
    {
        printf("up_left: %d, ", (A->up_left->data));
        printf("up_right: %d, ", (A->up_right->data));
        printf("down_left: %d, ", (A->down_left->data));
        printf("down_right: %d\n", (A->down_right->data));
    }
    for (auto &row: A->row_iterable())
    {
        for (auto &data: row)
            printf("%d - ", data);
        printf("\n");
...
//    for (auto row = A->row_begin(); row != A->row_end(); row++)
//    {
//        for (auto data = row->begin(); data != row->end(); data++)
//            printf("%d - ", *data);
//        printf("\n");
//    }
...
*/
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            if (j != 2) a[i][j]->right = a[i][j+1];
            if (j != 0) a[i][j]->left = a[i][j-1];
            if (i != 2) a[i][j]->down = a[i+1][j];
    ...
    int j = 0;
    for (auto i = x.begin(); i != x.end(); i++) assert(++j == *i);
    for (auto i = x.rbegin(); i != x.rend(); i++) assert(j-- == *i);
    assert(++x == y);
    z--;
    assert(y == z);
    assert(x-- != ++z);
...
void test_list_col() {
    ...
    auto x2 = l.insert_col(list_col_iter<int>(nullptr, nullptr), st, st);
    assert(x2 == l.col_end());
    auto x3 = l.insert_col(x1, st + 2, st + 5);
    auto x4 = l.insert_col(x1, st + 5, st + 6);
